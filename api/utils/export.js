...
export const exportAsPDF = (text, translated, sourceLang, targetLang, languageList) => {
  ...
  pdf.text(`Languages: ${sourceLang === 'en' ? 'English' : languageList.find(l => l.code === sourceLang)?.label} → ${languageList.find(l => l.code === targetLang)?.label}`, margin, yPosition);
  yPosition += lineHeight * 2;
    ...
    sections: [{
      ...
      children: [
        ...
        new Paragraph({
          children: [
            new TextRun({
              text: `Languages: ${sourceLang === 'en' ? 'English' : languageList.find(l => l.code === sourceLang)?.label} → ${languageList.find(l => l.code === targetLang)?.label}`,
              size: 24
            })
          ],
          spacing: { after: 400 }
